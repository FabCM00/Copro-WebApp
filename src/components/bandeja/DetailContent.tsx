        ...
        <div className="p-4 flex flex-col gap-5">
            ...
            {solicitud.sinMotor && (
                <div className="flex items-start gap-2.5 bg-amber-50 border border-amber-200 px-3 py-3 text-xs text-amber-800">
                    ...
                    <span>
                        Solicitud sin resultado de <span className="font-mono font-semibold">motor_process</span>.
            ...
            <Section title="Solicitante">
                ...
                <InfoRow label="Tipo identidad" value={fmtVal(v1Resp.tipoIden as string)} />
                <InfoRow label="Edad" value={v1Resp.edad_calculada != null ? `${v1Resp.edad_calculada} años` : undefined} />
                <InfoRow label="Fecha nacimiento" value={fmtFecha(v1Resp.fecha_nacimiento as string)} />
                <InfoRow label="Celular" value={celular} mono />
                <InfoRow label="Email" value={email} mono />
                <InfoRow label="Ocupación" value={fmtVal(v1Resp.ocupacion_copro_texto as string)} />
                <InfoRow
                    label="Antigüedad"
                    value={v1Resp.antiguedad_meses_calculada != null ? `${v1Resp.antiguedad_meses_calculada} meses` : undefined}
                />
                <InfoRow label="Fecha ingreso coop." value={fmtFecha(v1Resp.fecha_ingreso_coop as string)} />
                <InfoRow label="Fecha solicitud" value={solicitud.fecha} />
            </Section>

            {/* ── Solicitud / Oferta ────────────────────────────────── */}
            <Section title="Solicitud">
                <InfoRow label="Monto solicitado" value={fmtMoneda((v1Resp.req_amount as string) ?? (det.monto_credito as number))} highlight />
                {Object.keys(mpResp).length > 0 && (
                    <>
                        <InfoRow label="Neto final" value={fmtMoneda(mpResp.neto_final as number)} highlight />
                        <InfoRow label="Neto parcial" value={fmtMoneda(mpResp.neto_parcial as number)} />
                        <InfoRow label="Plazo" value={mpResp.plazo_meses != null ? `${mpResp.plazo_meses} meses` : undefined} />
                        <InfoRow label="Cuota (incluye seguro)" value={fmtVal(mpResp.cuota_incluye_el_seguro as string)} />
                        <InfoRow label="Cuota estatutarios" value={fmtMoneda(mpResp.cuota_mensual_estatutarios as number)} />
                        <InfoRow label="Tasa nominal M.V." value={fmtVal(mpResp.tasa_nominal_mes_vencido as string)} />
                        <InfoRow label="Modalidad" value={fmtVal(mpResp.modalidad as number)} />
                    </>
                )}
            </Section>
            ...
            {Object.keys(mpResp).length > 0 && (
                <Section title="Análisis del motor">
                    <InfoRow label="Ingreso total" value={fmtMoneda(det.ingreso_total as number)} />
                    <InfoRow label="Pasivos" value={fmtMoneda(mpResp.pasivos as number)} />
                    <InfoRow label="Gastos familiares" value={fmtMoneda(mpResp.gastos_familiares as number)} />
                    <InfoRow label="Capital en riesgo" value={fmtMoneda(mpResp.capital_riesgo as number)} />
                    <InfoRow label="Descuentos no bancarios" value={fmtMoneda(mpResp.descuentos_no_bancarios as number)} />
                    {mpResp.solvencia != null && <InfoRow label="Solvencia" value={(num(mpResp.solvencia) ?? 0).toFixed(2)} />}
                    {mpResp.capacidad_de_pago != null && <InfoRow label="Capacidad de pago" value={`${(num(mpResp.capacidad_de_pago) ?? 0).toFixed(2)}%`} />}
                    <InfoRow label="Endeudamiento Coprocenva" value={fmtMoneda(det.endeudamiento_coprocenva as number)} />
                    <InfoRow label="Obligaciones financieras" value={fmtMoneda(det.obligaciones_financieras as number)} />
                    <InfoRow label="Cuota externa Experian" value={fmtMoneda(det.cuota_externa_experian as number)} />
                    <InfoRow label="Deuda externa Experian" value={fmtMoneda(det.deuda_externa_experian as number)} />
                </Section>
            )}
            ...
            {(det.score_expe != null || det.score_trans != null) && (
                <Section title="Scoring (Centrales de riesgo)">
                    <InfoRow label="Score Experian" value={fmtVal(det.score_expe as string | number)} highlight />
                    <InfoRow label="Score TransUnion" value={fmtVal(det.score_trans as string | number)} />
                    <InfoRow label="Meses continuidad" value={fmtVal(det.meses_continuidad as number)} />
                    <InfoRow label="Mora externa 12m" value={fmtVal(det.mora_ext_12m as string)} />
            ...
            <Section title="Valida 1 — Criterios del cliente">
                ...
                <CriterioRow label="Validación Identidad" value={norm12(v1Resp.valida_id)} />
                <CriterioRow label="Validación Edad" value={norm12(v1Resp.valida_edad)} />
                <CriterioRow label="Validación Celular" value={norm12(v1Resp.valida_celular)} />
                <CriterioRow label="Validación Mora 12m" value={norm12(v1Resp.valida_mora_12m)} />
                <CriterioRow label="Validación Ocupación" value={norm12(v1Resp.valida_ocupacion)} />
                <CriterioRow label="Validación Antigüedad" value={norm12(v1Resp.valida_antiguedad)} />
                <CriterioRow label="Validación Endeudamiento" value={norm12(v1Resp.valida_endeudamiento_global)} />
            </Section>
            ...
            <Section title="Identidad — Validación documental y facial">
                {Object.keys(idResp).length > 0 ? (
                    <>
                        ...
                        />
                        <CriterioRow label="Documento de identidad" value={norm12(idResp.status_document)} />
                        <CriterioRow label="Validación facial (biometría)" value={norm12(idResp.status_face)} />
                        <CriterioRow label="Estado validación" value={norm12(idResp.estado_validacion)} />
                        <InfoRow label="Tipo de validación" value={idResp.tipo_validacion != null ? `Tipo ${idResp.tipo_validacion}` : undefined} />
            ...
            <Section title="Motor de crédito — Política de crédito">
                {Object.keys(mpResp).length > 0 ? (
                    <>
                        ...
                        />
                        <CriterioRow label="Cumple Solvencia" value={norm12(mpResp.cumple_solvencia)} />
                        <CriterioRow label="Cumple Capacidad de Pago" value={norm12(mpResp.cumple_capacidad_pago)} />
                        <CriterioRow label="Cumple Continuidad" value={norm12(mpResp.cumple_continuidad)} />
                        <CriterioRow label="Cumple Mora Externa 12m" value={norm12(mpResp.cumple_mora_ext_12m)} />
                        <CriterioRow label="Cumple Ingreso Total" value={norm12(mpResp.cumple_ingreso_total)} />
                        <CriterioRow label="Cumple Score Mínimo" value={norm12(mpResp.cumple_score_minimo)} />
                    </>
                ) : (
                    <p className="px-4 py-3 text-xs text-[#0D0D0D]/30 italic">
            ...
            <Section title="Envío a Coprocenva">
                {raw.coprocenva_envios ? (
                    <>
                        <div className="flex items-center justify-between gap-4 px-4 py-2.5">
                            <span className="text-xs text-[#0D0D0D]/45">Estado del envío</span>
                        ...
                        <InfoRow label="Status Coprocenva" value={fmtVal(envResp.status_coprocenva as number)} />
                        {typeof envResp.error === "string" && envResp.error && (
                            <div className="flex items-start gap-2.5 px-4 py-3 bg-red-50/60">
                                <AlertTriangle className="h-3.5 w-3.5 text-red-400 flex-shrink-0 mt-0.5" />
                                <p className="text-[11px] text-red-700 leading-relaxed break-all">{envResp.error}</p>
                            </div>
                        )}
                    </>
                ) : (
                    ...
                    {motivosNoApto.length > 0 ? (
                        <div className="divide-y divide-[#0D0D0D]/6">
                            {motivosNoApto.map((m, i) => (
                                <div key={i} className="flex items-start gap-2.5 px-4 py-3 bg-red-50/60">
                                    <AlertTriangle className="h-3.5 w-3.5 text-red-400 flex-shrink-0 mt-0.5" />
...
const MOTOR_JSON_PANELS: {
    ...
    { id: "identity",          shortLabel: "Identidad",        icon: <FileJson className="h-3.5 w-3.5" />,      hasReqRes: true },
    { id: "coprocenva_envios", shortLabel: "Envío Coprocenva", icon: <Send className="h-3.5 w-3.5" />,          hasReqRes: true },
...
export function MotorJsonView({ solicitud }: { solicitud: SolicitudUI; hideExpand?: boolean }) {
    ...
    const getPanelData = (panel: MotorJsonPanel, side: ReqResSide = "res"): unknown => {
        const raw = solicitud.raw;
        switch (panel) {
            case "valida1":
                return side === "req"
                        ...
                        if (!panel.hasReqRes) {
                            return (
                                ...
                                >
                                    ...
                                    <span className={`h-1.5 w-1.5 rounded-full flex-shrink-0 ${hasData ? "bg-emerald-500" : "bg-slate-200"}`} />
                        ...
                        return (
                            <Popover key={panel.id}>
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                        <span className={`flex-shrink-0 transition-colors ${isActive ? "text-[#012340]" : "text-slate-300"}`}>
                                            {panel.icon}
                                        </span>
                                        <span>{panel.shortLabel}</span>
                                        {isActive && (
                                        ...
                                        <span className="relative flex-shrink-0 h-3.5 w-3.5 flex items-center justify-center">
                                            {isActive ? (
                                                ...
                                                <span className={`h-1.5 w-1.5 rounded-full ${hasData ? "bg-[#012340]/25" : "bg-slate-200"}`} />
                                            )}
                                        </span>
                                        {isActive && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#012340]" />}
                                    </button>
                                </PopoverTrigger>
                                <PopoverContent
                                    side="bottom"
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        setPanelSide((prev) => ({ ...prev, [panel.id]: s }));
                                                    }}
                ...
                {activeData == null ? (
                    <div className="p-5">
                        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 px-4 py-4 text-sm text-amber-800">
                            ...
                            <p className="text-xs mt-1">No existe un registro asociado a este radicado.</p>
                        </div>
                    </div>
                ) : (
